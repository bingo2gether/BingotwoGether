  ...
  const handleFetchData = async (targetMode: 'incentive' | 'challenge' | 'prediction') => {
    ...
    try {
      if (targetMode === 'incentive') {
        ...
        const data = await getIncentive(p1.name, p2.name, contextData, seenIncentives);
        ...
        // Pass recent titles to avoid repetition
        const data = await getChallenge(p1.name, p2.name, seenChallenges);

        // Update history
        const newHistory = [...seenChallenges, data.title].slice(-20); // Keep last 20
